...
def getuvss(ufo, logger):
    # uvsdict = {}
    uvs = ufo.lib.get('org.sil.variationSequences', None)
    if uvs is not None:
        logger.log('in lib.plist org.sil.variationSequences is not supported, use public.unicodeVariationSequences', 'S')
    ...
    for g in ufo:
        ...
        if uvs is not None:
            logger.log('in .glif files org.sil.uvs is not supported, use public.unicodeVariationSequences in lib.plist', 'S')
    #         continue
